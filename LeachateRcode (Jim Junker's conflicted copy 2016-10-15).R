...
##Use this code to find your user name
#Sys.info()["user"]
##Place the output into where I have your name below

#if(Sys.info()["user"]=="Jim"){path="C:/Users/Jim/Dropbox/Leachate Experiment/Data Analysis"}
###if(Sys.info()["user"]=="Heidi"){path=...} #Place the correct path to your dropbox folder here
###if(Sys.info()["user"]=="Eric"){path=...}
###if(Sys.info()["user"]=="Jules"){path=...}
#setwd(path)
setwd("C:/Users/Jim/Dropbox/Leachate Experiment/Data Analysis")
...
doc<-read.csv(file="Leachate_TimeSeries1.csv",T)
doc<-doc[which(doc$Ctype!=""),]
pine<-doc[which(doc$Ctype=="Pine"),]
leaves<-doc[which(doc$Ctype=="Leaves"),]
grass<-doc[which(doc$Ctype=="Grasses"),]
resp2 <- read.csv(file = "Leachate_Respiration2.csv", T)
resp <-read.csv(file="Leachate_Respiration1_mod.csv",T)
o2_resp <- read.csv(file = "2018-06-30_O2-consump.csv", T)
doc_tot <- read.csv(file="Leachate_CumulativeJan2018.csv",T)
##New dataset that standardizes to % of carbon loss
doc <- mutate(doc, c("Ctype"), transform, Per = DOC/max(DOC)*100)
doc$log.Per <- log(doc$PerDOCrem)
doc$time.mod <- doc$Time + 1
...
doc$doc.cell <- doc$PerdDOC/(doc$Ncells/1000000)/doc$Time1
labile_avg = c()
for(i in 2:nrow(doc)){
  print(doc$labile[i])
...
#doc.change <- doc.change %>% group_by(Ctype) %>% mutate(U = abs(dDOC/(PerDOCrem*Time1)))
doc.change <- doc.change %>% group_by(Ctype) %>% mutate(U = abs((log(PerDOCrem)-log(shift(PerDOCrem,n=1,type = "lag")))/Time1))
...
str(o2_resp)
source("C:/Users/Jim/Documents/R/segmented.lme/source-segmented.lme.r")
library(nlme)
library(lme4)
...
pine.glm = gls(O2.consum ~ Days, correlation = corARMA(p=1,q=1), data = o2_resp.long)
...
plot(pine.glm);summary(pine.glm)  #plotting the linear model. Look @ standardized residuals and fitted values to estimate time of possible breakpoints
plot(CO2.accum~Time, data = resp2[resp2$Ctype == 'Pine',])
abline(pine.lm)
######  Quick test to check sensitivity of breakpoint to starting value #####
seg.pine1 = segmented(pine.glm, seg.Z = ~Days, psi = 5)
summary(seg.pine1)
seg.pine1$psi[[2]]
seg.pine2 = segmented(pine.glm, seg.Z = ~Days, psi = c(1, 40))
summary(seg.pine2)
seg.pine3 = segmented(pine.lm, seg.Z = ~Days, psi = c(.5,1,3))
summary(seg.pine3)
seg.pine5 = segmented(pine.lm, seg.Z =  ~Days, psi = c(1,5,15,20,37))
